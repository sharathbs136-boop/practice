...
payments = load_service("payments_app", "services/payments/src/app.py")
...
class ServiceWorkflowTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.servers = []
        for module in (inventory, products, payments):
            handler = module.InventoryHandler if module is inventory else module.ProductsHandler if module is products else module.PaymentsHandler
            server = module.ThreadingHTTPServer(("127.0.0.1", 0), handler)
            threading.Thread(target=server.serve_forever, daemon=True).start()
            cls.servers.append(server)
        inventory_port = cls.servers[0].server_address[1]
        payment_port = cls.servers[2].server_address[1]
        orders.INVENTORY_URL = f"http://127.0.0.1:{inventory_port}"
        orders.PAYMENT_URL = f"http://127.0.0.1:{payment_port}"
    ...
    def test_product_activation_and_order_deducts_stock(self):
        inventory_server, products_server, _, orders_server = self.servers
        product_events = queue.Queue()
        order_events = queue.Queue()
        products.SUBSCRIBERS.add(product_events)
        orders.SUBSCRIBERS.add(order_events)
        ...
        status, stock = self.request(inventory_server, "POST", "/stock/restock", {"sku": product["id"], "quantity": 4}, self.admin_headers)
        self.assertEqual(status, 200)
        ...
        self.assertEqual(order["reservation"]["remaining"], 6)
        status, updated = self.request(orders_server, "PUT", f"/admin/orders/{order['order_id']}/status", {"status": "shipped"}, self.admin_headers)
        self.assertEqual(status, 200)
        self.assertEqual(updated["status"], "shipped")
        self.assertEqual(product_events.get()[-1]["id"], "keyboard")
        self.assertEqual(order_events.get()[0]["order_id"], order["order_id"])
        products.SUBSCRIBERS.discard(product_events)
        orders.SUBSCRIBERS.discard(order_events)

        status, remaining = self.request(inventory_server, "GET", "/stock")
        self.assertEqual(status, 200)
        self.assertEqual(remaining[product["id"]], 6)

    def test_order_fails_when_stock_is_insufficient(self):
        inventory_server, _, _, orders_server = self.servers
    ...
    def test_order_stops_when_payment_is_declined(self):
        inventory_server, _, _, orders_server = self.servers
        self.request(inventory_server, "POST", "/stock/enable", {"sku": "keyboard", "quantity": 1}, self.admin_headers)
        status, response = self.request(orders_server, "POST", "/orders", {"sku": "keyboard", "quantity": 1, "payment_token": "declined"}, self.admin_headers)
