...
def emit_span(trace, name, started):
    if not ZIPKIN_URL:
        return
    payload = [{"traceId": trace, "id": uuid.uuid4().hex[:16], "name": name, "timestamp": int(started * 1_000_000), "duration": max(1, int((time.time() - started) * 1_000_000)), "localEndpoint": {"serviceName": "inventory"}}]
